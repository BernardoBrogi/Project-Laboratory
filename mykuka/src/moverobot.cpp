// Control the robot and move to a desire position
...
int main(int argc, char** argv)
{
    ...
    std::string action_service_name   =  param_name_value[node_name + "/action_service_name"];
    ...
    **/
    ...
    //Moving at a desired position
      ac::Joint_action move(nh);
      for(int i = 0; i<7; i++){
      ...
      //des_position  =  {Array};
      move.set_joint_values(des_position,ac::Joint_action::MESSAGE_TYPE::JOINT_POSITION);
      move.debug_print = true;
      actions["move"]         = &move;
     ...
     */
     ac::Action_client_cmd_interface action_cmd_interface(nh,kuka_action_client,action_service_name,cmd_service_name);
